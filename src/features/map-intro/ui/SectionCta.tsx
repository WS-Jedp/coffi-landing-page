...
import { toPurposeId } from "@/models/coffiFilters";
 ...
 * one. `IntentId` values were chosen to BE the `purpose` values the app
 * accepts — that alignment was the reason for choosing them — so somebody who
 * clicked "Enfoque" here lands in the app already filtered for it instead of on
 * a blank search.
 *
 * `FUN` is the exception, and `toPurposeId` is what handles it: the product has
 * no group for going out, so that intent forwards no purpose rather than one
...
export const SectionCta: React.FC<{ intent: IntentId | null }> = ({ intent }) => {
  ...
  return (
    <div className="mt-6 md:mt-10">
      <button
        type="button"
        onClick={() => redirectToCoffiWithFilters({ purpose: toPurposeId(intent) })}
